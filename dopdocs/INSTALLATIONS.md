...
* На всех серверах кластера, ceph не был установлен. Отсутствует LVM на дисках сервера.
* В PuppetDB отсутствуют сертификаты для серверов кластера ceph
* В PuppetDB отсутствует экспортированный файл ceph.conf

Если модуль ceph применяется в вычислительной среде, где ранее развёртывался ceph, то результат применения модуля трудно прогнозируемый. В этом случае необходимо, как минимум, удалить сертификаты серверов кластера ceph, удалить экспортированный файл ceph.conf, удалить ранее созданные ceph LVM c дисков сервера.

Для удаления сертификатов, необходимо на Cервере управления в командной строке выполнить:
...
Для удаления экспортированного файла ceph.conf, необходимо на Cервере управления в командной строке выполнить:

```{.bash}
sudo puppet node deactivate "fqdn_first_mon"
```

где "fqdn_first_mon" - FQDN сервера ceph (основного интерфейса сервера, не интерфейса передачи данных, без перфикса "ib"), на котором ранее экспортировался файл ceph.conf (этот сервер был первым MON), например, cephmon07.ppoi.ensk. Вместо "fqdn_first_mon" одного сервера может быть указано регулярное выражение описывающее "fqdn" нескольких серверов, например, pnode{101..116}.ppoi.ensk.

Для удаления ранее созданные ceph LVM c дисков сервера на всех серверах на которых будут развертываться OSD, в командной строке выполнить:
...
!!! Не работает пока. Вместо ручного удаления lvm, можно установить параметр `$forceDelLvmOsdHdd` в `true` в манифесте init.pp модуля ceph.
...
Перед загрузкой модуля ceph в СПО КВР нужно убедиться, что в манифестах заданы значения по умолчанию следующим параметрам:

 Манифест   | Параметр манифеста   | Значение параметра
----------- | -------------------- | ------------------
...
Далее, следуя инструкциям на СПО КВР:
...
Для создания pools нельзя применять модуль ceph на сервере имеющем роль MON или MDS (надо проверить, это было давно уже).
...
1. После успешного создания pools есть возможность приступить к монтированию директорий:
   ...
   * В КСУ у ресурса `ceph` заполнить параметр `all_mon_name_data` именами интерфейса передачи данных (Infiniband) всех MONs. Имена необходимо заключить в кадратные скобки и внутри скобок каждое имя в одинарные кавычки, например: ['cephmon07ib', 'cephmon06ib', 'cephmon05ib'].
